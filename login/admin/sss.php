...
$id = $_POST['select'];
$conn->prepare("DELETE FROM sss WHERE id = ?")->execute([$id]);
}
...
$guncelle = $conn->prepare("UPDATE sss SET baslik=:baslik,icerik=:icerik WHERE id=:id");
...
<html lang="en">
 ...
 <body class="dashboard dashboard_1">
          ...
          <nav id="sidebar">
             ...
             <div class="sidebar_blog_2">
                ...
                <ul class="list-unstyled components">
                   ...
                   <li><a href=""><i class="fa fa-clock-o orange_color"></i> <span>S.S.S</span></a></li>
                </ul>
             </div>
          </nav>
          <!-- end sidebar -->
  ...
  align-items:center;">
      ...
      <div class="user-nick">
      <h6>Açıklama</h6>
      <textarea name="icerik" id="fiyat" cols="25" rows="2">  <?php echo $row['icerik'] ?></textarea>
      </div>
      <a href="sss.php?duzenle=<?php echo $row['id'] ?>" class="btn btn-info" style="width:90px; text-align: center;
  height: 40px;">Düzenle</a>  

      <a href="sss.php?sil=<?php echo $row['id'] ?>" class="btn btn-danger" style="width: 90px;
  text-align: center;
  height: 40px;">Sil</a>
  </div>
  <hr>
...
if (isset($_GET['sil'])) {
  $sil = $conn->prepare("DELETE FROM sss WHERE id=:id");
...
if (isset($_GET['duzenle'])) {
  $duzenle = $conn->prepare("SELECT * FROM sss WHERE id=:id");
  ...
  <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group" style="    margin-bottom: 5px;">
          ...
          <input type="text" class="form-control" name="icerik" id="fiyat" value="<?php echo $row['icerik'] ?>">
      </div>
...
$conn->prepare("INSERT INTO `sss`(`baslik`,`icerik`) VALUES (?,?)")->execute([$baslik,$aciklama]);
  ...
  <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group" style="    margin-bottom: 5px;">
          <label for="unvan">Başlık</label>
          <input type="text" class="form-control" name="baslik" id="fiyat" value="" placeholder="Soru Ekleyiniz">
      </div>
      <div class="form-group" style="    margin-bottom: 5px;">
          <label for="unvan">Açıklama</label>
          <input type="text" class="form-control" name="icerik" id="fiyat" value="" placeholder="Açıklama Ekleyiniz">
      </div>
